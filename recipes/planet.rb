...
cron 'update planet' do
  ...
  command "/usr/local/bin/lockrun --lockfile /tmp/.update_planet.lock -- #{node[:mapzen_odes][:setup][:scriptsdir]}/update_planet.sh >#{node[:mapzen_odes][:setup][:basedir]}/logs/update_planet.log 2>#{node[:mapzen_odes][:setup][:basedir]}/logs/update_planet.err"
  user    node[:mapzen_odes][:user][:id]

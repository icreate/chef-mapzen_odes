...
cron 'update polygons' do
  ...
  command "/usr/local/bin/lockrun --lockfile /tmp/.update_polygons.lock -- #{node[:mapzen_odes][:setup][:scriptsdir]}/update_polygons.sh >#{node[:mapzen_odes][:setup][:basedir]}/logs/update_polygons.log 2>#{node[:mapzen_odes][:setup][:basedir]}/logs/update_polygons.err"
  user    node[:mapzen_odes][:user][:id]
